...
pub fn abbrev(input: &[u8]) -> Result<RzBinDwarfDebugAbbrevs, ()> {
    unsafe {
...
pub fn info(input: &[u8], big_endian: bool,
            abbrev: *mut RzBinDwarfDebugAbbrevs,
            str: Option<*mut RzBinDwarfDebugStr>) -> Result<RzBinDwarfDebugInfo, ()> {
    unsafe {
        let buf = rz_buf_new_with_bytes(input.as_ptr(), input.len() as c_ulonglong);
        if buf.is_null() {
            return Err(());
...
pub fn line(input: &[u8], big_endian: bool,
            encoding: *mut RzBinDwarfEncoding,
            mask: RzBinDwarfLineInfoMask,
            info: *mut RzBinDwarfDebugInfo)
    ...
    unsafe {
        ...
        let x = rz_bin_dwarf_line_from_buf(
            buf, big_endian, encoding, info, mask);
        if x.is_null() {
            return Err(());
        }
        Ok(*x)
